...
namespace Egl {
	...
	void OpenGLFramebuffer::Invalidate() {
		...
		if (mColorAttachmentSpecifications.size() != 0) {
			...
			CreateTextures(hasMultipleSamples, (uint32_t)mColorAttachments.size(), mColorAttachments.data());

			for (size_t i = 0; i < mColorAttachments.size(); i++) {
				BindTexture(hasMultipleSamples, mColorAttachments[i]);
				switch (mColorAttachmentSpecifications[i].format) {
				case FramebufferTextureFormat::RGBA8:
					AttachColorTexture(mColorAttachments[i], mDefenition.samples, GL_RGBA8, GL_RGBA, mDefenition.width, mDefenition.height, (int)i);
					break;
				case FramebufferTextureFormat::RED_INT:
					AttachColorTexture(mColorAttachments[i], mDefenition.samples, GL_R32UI, GL_RED_INTEGER, mDefenition.width, mDefenition.height, (int)i);
					break;
				}
			}
		}
