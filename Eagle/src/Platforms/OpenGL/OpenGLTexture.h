...
namespace Egl {
	Texture::Texture(uint32_t width, uint32_t height, bool scaleUpBlur) : mWidth(width), mHeight(height) {
	...
	Texture::Texture(const std::string& path, bool scaleUpBlur) : mPath(path), mDataFormat(0), mInternalFormat(0) {
